...
function HomeRoute() {
  ...
  const { data: python } = useRepositories("stars:>10000 language:python");
  const { data: java } = useRepositories("stars:>10000 language:java");
  ...
  return (
    <div>
      ...
      <div className="container mx-auto py-8 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <RepositoriesTable label="Most Popular Python" repositories={python} />
        <RepositoriesTable label="Most Popular Java" repositories={java} />
      </div>
    </div>
  );
}
